...
Mesh Mesh::MakeCube()
{
    ...
    // (Duplicate vertices at edges so each face gets its own normal)
    const float h = 0.5f;
    ...
    {
        unsigned base = f * 4;
        // Two triangles per quad face (CCW winding)
...
Mesh Mesh::MakePlane(float size, int subdivisions)
{
    ...
    int n = subdivisions + 1;  // Vertex count per side
