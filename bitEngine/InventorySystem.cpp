...
{
    // Stack with an existing entry of the same name
...
// ── DrawUI ────────────────────────────────────────────────────
// Deus Ex: Mankind Divided-style overlay:
//   - Full-screen semi-transparent dark background
//   - Left panel: category list (future; currently just "All")
//   - Main area: scrollable item grid (name + quantity + description)
...
void InventorySystem::DrawUI(float displayW, float displayH)
{
    ...
    bg->AddRectFilled({0, 0}, {displayW, displayH},
                      IM_COL32(0, 0, 0, 180));
                     ...
                     ImGuiWindowFlags_NoScrollbar))
    {
        ...
        if (m_entries.empty())
        {
            ImGui::TextColored(ImVec4(0.45f, 0.42f, 0.38f, 1.f),
                               "Your inventory is empty.");
        }
        else
        {
            // ── Item table ────────────────────────────────────
            // Columns: Name | Qty | Description
            ImGui::BeginChild("InvScroll", ImVec2(0.f, 0.f), false);

            // Column widths
            const float kQtyW  = 48.f;
            const float kDescW = kPanelW - 200.f;  // remainder
